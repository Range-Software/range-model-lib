                     ...
                     double         tolerance)
{
    ...
    {
        throw RError(RError::Type::ReadFile,R_ERROR_REF,"Failed to read the file \'%s\'. %s",fileName.toUtf8().constData(),rError.getMessage().toUtf8().constData());
    }
...
void RModelRaw::readTextStream(QTextStream &textSTream, double tolerance)
{
    ...
    static QRegularExpression rx("[,; ]");// match a comma or semicolon or a space
    ...
    while (!textSTream.atEnd())
    {
        ...
        {
            throw RError(RError::Type::InvalidInput,R_ERROR_REF,"Failed to read the text stream.");
        }
    }
