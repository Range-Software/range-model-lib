...
const RVersion RModel::version = RVersion(FILE_MAJOR_VERSION,FILE_MINOR_VERSION,FILE_RELEASE_VERSION);
...
void RModel::read(const QString &fileName)
{
    ...
    while (!targetFileName.isEmpty())
    {
        ...
        {
            throw RError(RError::Type::Application,R_ERROR_REF, "Memory allocation failed.");
        }
        catch (const std::exception&x)
        {
            throw RError(RError::Type::Application,R_ERROR_REF, "%s.", typeid(x).name());
        }
        catch (...)
        {
...
std::vector<uint> RModel::findElementIDs(REntityGroupType entityType, const std::vector<uint> entityIDs) const
{
    ...
    for (uint i=0;i<entityIDs.size();i++)
    {
        ...
        if (groupID == RConstants::eod)
        {
            throw RError(RError::Type::Application,R_ERROR_REF,"Invalid entity ID.");
        }
        const RElementGroup *pElementGroup = this->getElementGroupPtr(groupID);
        if (!pElementGroup)
        {
            throw RError(RError::Type::Application,R_ERROR_REF,"Invalid entity ID. Element group was not found.");
        }
        for (uint j=0;j<pElementGroup->size();j++)
        {
...
QVector<bool> RModel::findEdgeNodes() const
{
    ...
    uint64_t nodeGroupCounter = 1;

    uint64_t nNodeGroupCounter = this->getNPoints() + this->getNLines() + this->getNSurfaces() + this->getNVolumes();
...
uint RModel::fixSliverElements(double edgeRatio)
{
    ...
    for (uint i=0;i<this->getNElements();i++)
    {
        ...
        if (lMin <= RConstants::eps || (lMax/lMin) >= edgeRatio)
        {
            ...
            {
                RLogger::unindent();
                throw RError(RError::Type::Application,R_ERROR_REF,"Failed to merge nodes %u and %u. %s",n1,n2,rError.getMessage().toUtf8().constData());
            }
...
QList<uint> RModel::findIntersectedElements() const
{
    ...
    const uint64_t n_total = uint64_t(this->getNElements()) * (this->getNElements() - 1) / 2;
    std::atomic<uint64_t> n_count(0);
...
#pragma omp critical
        ...
        if ((i % 100) == 0)
        {
            ...
            {
                RProgressPrint(n_count.load(), n_total);
            }
        }

        for (int64_t j=i+1;j<int64_t(this->getNElements());j++)
        {
            n_count.fetch_add(1, std::memory_order_relaxed);
            if ((intElements[i] && intElements[j]) || !RLimitBox::areIntersecting(limitBoxes[i],limitBoxes[j]))
            {
                continue;
            }
...
bool RModel::boolDifference(uint nIterations, QList<uint> surfaceEntityIDs, uint cuttingSurfaceEntityId)
{
    ...
    {
        throw RError(RError::Type::Application,R_ERROR_REF,"Failed to break intersected elements. %s", error.getMessage().toUtf8().constData());
    }
    ...
    for (int i=0;i<surfaceEntityIDs.size();i++)
    {
        ...
        {
            throw RError(RError::Type::Application,R_ERROR_REF,"Failed to check if element centers are inside surface. %s",
...
uint RModel::tetrahedralizeSurface(const std::vector<uint> surfaceIDs)
{
    ...
    {
        RLogger::unindent();
        throw RError(RError::Type::Application,R_ERROR_REF,"Failed to tetrahedralize surface. %s", error.getMessage().toUtf8().constData());
    }
    ...
    for (uint surfaceID=0;surfaceID<this->getNSurfaces();surfaceID++)
    {
        ...
        for (uint i=0;i<rSurface.size();i++)
        {
            ...
            if (!rElement.findArea(this->nodes,elementArea))
            {
                ...
                throw RError(RError::Type::Application,R_ERROR_REF,"Could not calculate element area\n");
            }
...
QString RModel::readAscii(const QString &fileName)
{
    ...
    {
        throw RError(RError::Type::InvalidFileFormat,R_ERROR_REF,"File type of the file \'" + fileName + "\' is not MODEL.");
    }

    RLogger::debug("File version: %s (target: %s)\n",
                   fileHeader.getVersion().toString().toUtf8().constData(),
                   RModel::version.toString().toUtf8().constData());
...
void RModel::writeAscii(const QString &fileName) const
{
    ...
    uint64_t nsteps = this->getNNodes()
                    + this->getNElements()
                    + this->getNPoints()
                    + this->getNLines()
                    + this->getNSurfaces()
    ...
    RFileHeader fileHeader(R_FILE_TYPE_MODEL,RModel::version);
    RLogger::debug("File header: %s\n",fileHeader.toString().toUtf8().constData());
...
#pragma omp critical
        {
            RProgressPrint(++nProcessed,uint64_t(volumeElements.size()));
        }
